function generateAristaConfig(config) {
    let output = `! Arista Configuration\n`;

    output += `! AAA Configuration\n`;
    switch (config.aaa.authMethod) {
        case '1': output += `aaa authentication login default local\n`; break;
        case '2': output += `aaa authentication login default group radius local\n`;
                  output += `aaa authentication dot1x default group radius\n`; break;
        case '3': output += `aaa authentication login default group tacacs+ local\n`;
                  output += `aaa authentication dot1x default group radius\n`; break;
        case '4': output += `aaa authentication login default group tacacs+ group radius local\n`;
                  output += `aaa authentication dot1x default group radius\n`; break;
    }
    switch (config.aaa.authorMethod) {
        case '2': output += `aaa authorization network default group radius\n`; break;
        case '3': output += `aaa authorization commands 15 default group tacacs+ local\n`; break;
        case '4': output += `aaa authorization commands 15 default group tacacs+ local\n`;
                  output += `aaa authorization network default group radius\n`; break;
    }
    switch (config.aaa.accountMethod) {
        case '2': output += `aaa accounting network default start-stop group radius\n`; break;
        case '3': output += `aaa accounting commands 15 default start-stop group tacacs+\n`; break;
        case '4': output += `aaa accounting commands 15 default start-stop group tacacs+\n`;
                  output += `aaa accounting network default start-stop group radius\n`; break;
    }
    output += `!\n`;
    ...
    if (config.dot1x.enable === '1') {
        ...
        output += ` dot1x max-req ${config.dot1x.maxReauth}\n`;
        output += ` dot1x reauthentication\n`;
        output += ` dot1x timeout reauth-period ${config.dot1x.reauthPeriod}\n`;
